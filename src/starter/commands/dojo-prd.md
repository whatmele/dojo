---
description: Produce a product requirements document for the active session.
argument-hint: [feature / user problem / goal]
scope: session
---
...
If any item cannot be reasonably inferred, **do not guess**. Reply with missing points and **2–4 concrete questions**. If you cannot multi-turn, write a `clarifications-needed.md` (or similar) under `${artifact_dir:product-requirement}` and stop until the user fills gaps.

Proceed to Step 2 only when goals are clear enough.

## Step 2 — Write the PRD

After validation, add structured PRD Markdown under `${artifact_dir:product-requirement}` (e.g. `PRD.md`, plus appendices if useful). Suggested sections:
...
2. **Only** write under `${artifact_dir:product-requirement}`.
