...
- 生成适配不同 AI 工具的 command 文件，并通过软链接统一分发
...
│   (symlink)  (source)  (symlink) (symlink)       │
└──────┬───────────┬───────┬───────┬───────────────┘
       │           │       │       │
       └───────────┴───┬───┴───────┘
                       │
...
Command 是预定义的 prompt 模板，存放在 `.dojo/commands/` 目录下（`.md` 文件）。通过 `dojo session new`、`dojo session resume` 或 `dojo context reload` 时自动生成到 `.agents/commands/` 目录。

生成过程中 `${dojo_current_session_id}` 占位符被替换为当前活跃 Session 的 ID。`$ARGUMENTS` 由 AI 工具自身负责替换（用户触发 command 时传入的文本），Dojo 不处理。
...
├── .claude/commands/                # ← 软链接到 .agents/commands/
├── .trae/commands/                  # ← 软链接到 .agents/commands/（格式兼容时）
│
...
## 仓库说明
[静态/半自动] 各仓库的职责和依赖关系。详细通信协议参见 docs/ 目录。
...
| 命令 | 说明 |
|------|------|
| `dojo init` | 初始化工作区：交互式配置工作区名称、描述、选择 AI coding 工具；生成 `.dojo/` 目录结构（含 `config.json`、`state.json`、默认 commands）；生成 `AGENTS.md` 模板、`.gitignore`；初始化工作区 Git 仓库 |
...
| 命令 | 说明 |
|------|------|
| `dojo context reload` | 刷新上下文 **和** commands：重新生成 `.dojo/context.md`、更新 `AGENTS.md` 动态段、重新从 `.dojo/commands/` 生成 `.agents/commands/`（替换占位符）。这是一个原子操作，确保所有动态内容同步到最新状态 |

### 5.5 启动 AI 工具

| 命令 | 说明 |
|------|------|
| `dojo start [tool]` | ① 检查是否有 active session（无则提示先执行 `dojo session new`，退出） ② 执行 `context reload` ③ 启动指定的 AI coding 工具（默认取 `config.json` 中 agents 列表第一个）。例如 `claude`、`codex` 等 |
...
.dojo/commands/*.md        ← 源文件（含 ${dojo_current_session_id} 占位符）
       │
       │  session new / session resume / context reload 时
       │  读取源文件 → 替换 ${dojo_current_session_id} → 输出到 ↓
       ▼
.agents/commands/*.md      ← 生成的目标文件（占位符已替换为实际 session ID）
       │
       ├── symlink ← .claude/commands/
       └── symlink ← .trae/commands/（格式兼容时）
```

### 分发策略

- `.agents/commands/` 是生成目标，存放占位符已替换的 `.md` 文件
- 只为 `config.json` 中 `agents` 列表声明的、且命令格式兼容的工具创建软链接
...
| 占位符 | 替换者 | 替换时机 |
|--------|--------|----------|
| `${dojo_current_session_id}` | Dojo CLI | `session new` / `session resume` / `context reload` |
...
| `dojo start` 无 active session | 提示用户先执行 `dojo session new`，退出 |
...
| 默认 commands | 内置 prd、research、tech-design、task-decompose、dev-loop、review、commit、gen-doc 共 8 个模板 |
